...
This drops `laser-device.plugin` (macOS) / `laser-device.dll` (Windows) into
...
└── laser_device.dll        # Windows x64 binary
```
...
with two-way bindings. Binaries are platform-specific, so ship both files
alongside the .tox when distributing.
...
- **Input**: one CHOP, one sample per laser point. Channels are matched by
  ...
  (x, y, r, g, b, i). Coordinates are in [-1, 1]; colors in [0, 1]. This is
  the same layout TouchDesigner's native laser tools produce.
...
- x/y are hard-clamped to [-1, 1] and a blank frame is sent on teardown,
  deactivation and input loss.
